  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      <div className="pt-32 pb-12 px-6 text-center border-b border-gray-100">
        <h1 className="text-5xl md:text-7xl font-serif font-bold text-gray-900 tracking-tight mb-4">Stage Setup</h1>
          ...
          <div className="sticky top-32">
            ...
            <ul className="space-y-4">
              {quickLinks.map((link, idx) => (
                <li key={idx}>
                  <button onClick={() => navigate(`/${link.toLowerCase().replace(/\s+/g, '-')}`)} className={`text-sm font-bold uppercase tracking-widest transition-all duration-300 hover:text-pink-900 hover:translate-x-2 cursor-pointer w-full text-left ${link === 'Stage' ? 'text-pink-900' : 'text-gray-400'}`}>
